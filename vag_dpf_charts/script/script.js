...
function parseContent(content)
{
	...
	arrayHeader.pop();
	//console.log("Имена полей ассоциативного массива: ", arrayHeader);
	...
	for (let n=0; n<arrayHeader.length; n++)
	{
		logData[arrayHeader[n]] = [];
	
		for (let i=0; i<strArray.length; i++)
			{
				let tempArrayStrings = strArray[i].split(/\t/);
				logData[arrayHeader[n]].push( Number(tempArrayStrings[n]) );
			}
	}
...
//Функция преобразования значений Time (ms), миллисек. в минуты прошедшие от текущего времени
//Добавляем полученные значения в многомерный массив
function transformTime()
{
	arrayHeader.push(headerTransformTime);
	logData[headerTransformTime] = [];

	for (let i=0; i<logData['Time (ms)'].length; i++)
	{
		logData[headerTransformTime][i] = Math.round( (logData['Time (ms)'][i] - Date.now()) / 1000 / 60 / 60) ;
		//console.log(logData[headerTransformTime]);
		//logData['Time (ms)'][i] = Date.now().toLocaleTimeString();
	}
}


//Функция изменения из многомерного массива нулевых (ошибок) значений. Ищет по массиву пробега
function deleteZeroElements()
{
	for (let n=0; n<logData['Distance driven (km)'].length; n++){

		if (logData['Distance driven (km)'][n]==0)
		{
			for (let i=0; i<arrayHeader.length; i++)
			{
				logData[arrayHeader[i]][n] = logData[arrayHeader[i]][n-1];		
			}
		}
	}
}
...
function createСheckedOptionsGraf()
{
	...
	elem = document.getElementById('Distance Since Last Regen. (km)');
	elem.getElementsByClassName('radioBut_Y_2')[0].checked = true;

	elem = document.getElementById('Oil Ash Residue (g)');
